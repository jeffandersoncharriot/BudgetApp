...
namespace BudgetCodeTests
{
    ...
    public class TestDatabase
    {
        ...
        [Fact]
        public void SQLite_TestNewDatabase_newDBDoesExist_shouldHaveData()
        {
            // Arrange
            string path = TestConstants.GetSolutionDir();
            string filename = "newdb.db";
            ...
            // Modified: The following was mentioned in our Sprint 1 presentation;
            //           Our setup adds the categoryTypes table no matter what
            //           because it's an enum and should never be absent,
            //           so instead of testing for an empty table we test for a
            //           *non-empty* one.
            Assert.True(rdr.HasRows, "There is no data in the new databse"); // The category types are put in database on initialization
        ...
        public void SQLite_TestNewDatabase_ForeignKeyConstraintsEnabled()
        {
            ...
            // Jordan: Changed from "Foreign Keys=1" because of
            //         ConnectionStringBuilder.
            //         Out of an ambundance of caution, I also created another
            //         test which actually checks that foreign keys are
            //         working.
            Assert.True(connectionString.Contains("foreign keys=True"), "FK Constraints enabled");

        }

        [Fact]
        public void SQLite_TestNewDatabase_ForeignKeyConstraintsActuallyWork()
        {
            // I'm paranoid and want to make sure the foreign keys are actually working
            ...
            // -1 is not a valid ID:
            cmd.CommandText = "INSERT INTO expenses VALUES (1, '2022-01-01', 0.0, 'exp name', -1)";
		...
		[Fact]
        public void SQLite_TestExistingDatabase_ShouldThrowWhenFileDoesNotExist() {
            // Arrange
            string dir = TestConstants.GetSolutionDir();
            string filename = "dont_use_this_filename.db";
            ...
            bool threw = false;
            try {
                Database.existingDatabase(fullPath);
            } catch {
                threw = true;
            }
            ...
            // Assert
            Assert.True(threw);
            Assert.Null(Database.dbConnection);
        }
